...
func _make_background_texture() -> Texture2D:
	...
	var top: Color = Color(0.08, 0.08, 0.12)
	var bottom: Color = Color(0.18, 0.16, 0.2)
	for y in range(img.get_height()):
		var t: float = float(y) / float(img.get_height() - 1)
...
func _add_stars(img: Image, count: int) -> void:
	...
	for i in range(count):
		var x: int = randi_range(0, w - 1)
		var y: int = randi_range(0, int(h * 0.4))
...
func _add_hills(img: Image) -> void:
	...
	var h: int = img.get_height()
	for x in range(w):
		var height: int = int(10 + 6 * sin(float(x) * 0.08) + 4 * sin(float(x) * 0.2))
